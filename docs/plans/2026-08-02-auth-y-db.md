...
| `internal/user/user.go` | Modelo, normalización de email, validación |
| `internal/user/store.go` | CRUD sobre SQLite |
| `internal/user/*_test.go` | Validación y CRUD |
...
Run: `go test ./internal/user/ -v` → FAIL con `undefined: Validar`

- [ ] **Step 3: Implementar `user.go`**
...
**Interfaces:**
- Consumes: `User`, `NormalizarEmail` de la Task 2; `storagetest.AbrirMigrada` de la Task 1.
- Produces:
  - `type Store struct{ … }`, `func NewStore(db *sql.DB) *Store`
  - `func (s *Store) Crear(ctx context.Context, name, email, hash string) (*User, error)`
  - `func (s *Store) PorEmail(ctx context.Context, email string) (*User, string, error)` — usuario **y** hash
  - `func (s *Store) PorID(ctx context.Context, id int64) (*User, error)`
...
func TestUserNoExponeElHash(t *testing.T) {
	...
	// El hash sólo viaja por el retorno explícito, nunca dentro de User.
	if s := fmt.Sprintf("%+v", *u); strings.Contains(s, "SECRETO") {
		t.Errorf("el hash apareció en User: %s", s)
	}
}
```
...
Run: `go test ./internal/user/ -run "TestCrear|TestPor|TestUser" -v` → FAIL con `undefined: NewStore`
...
// User a propósito: así no puede filtrarse al renderizar o serializar.
func (s *Store) PorEmail(ctx context.Context, email string) (*User, string, error) {
	var (
		u     User
		hash  string
		creado int64
...
```bash
git add internal/user/store.go internal/user/store_test.go
...
**Interfaces:**
- Consumes: `Sessions` (Task 5), `user.Store` (Task 3).
- Produces:
  - `const NombreCookie = "zapping_session"`
  - `type Guard struct{ … }`, `func NewGuard(s *Sessions, u *user.Store, cookiesSeguras bool) *Guard`
  ...
  - `func UsuarioDe(ctx context.Context) (*user.User, bool)`
...
func NewGuard(s *Sessions, u *user.Store, cookiesSeguras bool) *Guard {
...
func UsuarioDe(ctx context.Context) (*user.User, bool) {
	u, ok := ctx.Value(claveUsuario).(*user.User)
	return u, ok
}
...
- [ ] La contraseña nunca aparece en la base, ni en `User`, ni en el hash.
