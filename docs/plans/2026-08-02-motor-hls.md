...
- El reloj se inyecta siempre como `func() time.Time`. La lógica del ciclo se prueba con reloj inyectado; **ningún test espera más de 1 segundo de tiempo real**. Los tests de `Run` usan timers reales sobre un pool de duraciones en milisegundos.
...
### Task 4: Snapshot — ventana, discontinuidad y render del playlist

Construcción del snapshot y render del `.m3u8` van juntos: `discAt` sólo existe para el render, así que separarlos obligaría a un stub muerto.
...
- Produces:
  ...
  - `func renderPlaylist(p *Pool, s *Snapshot, discAt int) []byte` (privada)
...
Run: `go test ./internal/hls/ -run "TestBuildSnapshot|TestRenderPlaylist" -v`
...
- [ ] **Step 4: Implementar `renderPlaylist`**

Al final de `internal/hls/snapshot.go`:
...
Run: `go test ./internal/hls/ -v`
Expected: PASS en los 9 tests de snapshot

- [ ] **Step 6: Commit**

```bash
git add internal/hls/snapshot.go internal/hls/snapshot_test.go
git commit -m "feat(hls): snapshot inmutable con playlist pre-renderizado"
```

---

### Task 5: Engine — publicación atómica y lectura sin locks
...
### Task 6: Engine — bucle de rotación
...
**Interfaces:**
- Consumes: `Engine.refresh()` de la Task 5.
...
Verificables al terminar la Task 6:
